...
export function testExperienceMain(div: HTMLDivElement, locale: Locale) {
  ...
  const liNormal = divContainer.querySelectorAll(
    'ul.timeline>li:not(.timeline-inverted)'
  )
  ...
  const liInverted = divContainer.querySelectorAll(
...
export function testJobResponsibilities(div: HTMLDivElement): void {
  expect(div.querySelector('h5')).toHaveTextContent(
    'business-card-experience-responsibilities-title'
  )
...
export function testJobTechnologies(
  div: HTMLDivElement,
  technologies: string[]
): void {
  ...
  expect(div.querySelector('h5')).toHaveTextContent(
    'business-card-experience-technologies-title'
  )

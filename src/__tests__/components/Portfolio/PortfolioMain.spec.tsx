...
test('should render PortfolioMain correctly', () => {
  ...
  for (let i = 1; i < 34; i += 2) {
    expect(tMock).toHaveBeenNthCalledWith(i + 1, expect.stringMatching(/^business-card-portfolio-.*-title$/));
    expect(tMock).toHaveBeenNthCalledWith(i + 2, expect.stringMatching(/^business-card-portfolio-.*-subtitle$/));
  }
});
